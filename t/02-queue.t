...
ok ( my $r = Resque->new( namespace => 'test_resque' ), 'Build test object' );
ok ( $r->redis, 'Has redis object' );

SKIP: {
    skip "Can't connect to redis server" unless $r->redis->ping;

    diag('Testing on connected redis server...');
    $r->flush_namespace;
